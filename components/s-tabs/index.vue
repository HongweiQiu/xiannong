				...
				<div class="scroll-view">
					<div class="s-tab-nav-view">
						<template v-if="!slotTitle">
							<div v-for="(item,index) of navInfoList"
								:class="['s-tab-nav',{'is-disabled':item.disabled},{'is-active':active==index}]"
								:style="navWidth+'color:'+(active==index?activeColor:color)" :key="index"
						...
						query().select('.s-tab-nav-view').boundingClientRect().exec(([view]) => {
							...
							const setNavScroll = (item) => {
								if (item) {
									...
									this.scrollLeft = Math.abs(view.left - wrap.left - this
							...
							if (this.slotTitle) {
								...
								uni.createSelectorQuery().in(this.navContextList[this.active])
									.select('.s-tab-nav').boundingClientRect().exec(([item]) => {
										setNavScroll(item);
									});
							} else {
								query().selectAll('.s-tab-nav').boundingClientRect().exec(([
									list]) => {
