<template>
<uni-shadow-root class="slider-index"><view class="pathway" @touchend="onEnd" id="pathway">123
    <view class="tips">
        <text v-if="isOk" style="color: #FFFFFF;">验证通过</text>
        <text v-else>向右滑动完成验证</text>
    </view>
    <view class="track" :style="'transform:translateX('+(oldx)+'px)'"></view>
    <movable-area>
        <movable-view :x="x" direction="horizontal" @change="onChange" :class="isOk ? "active":""" id="track">
        </movable-view>
    </movable-area>
